const buildRepeatKeyString = (key, length) =>
  Array.from({ length })
    .fill(key)
    .join("");
  ...
  context("values", () => {
    context("required Helm values entered", () => {
      it("successfully saves Helm values", () => {
        const downArrowsToRequiredHelmValue = buildRepeatKeyString(
          "{downarrow}",
          16
        );
        const rightArrowsToRequiredHelmValue = buildRepeatKeyString(
          "{rightArrow}",
          16
        );
  ...
  context("kustomize", () => {
    context("valid line clicked in editor", () => {
      it("generates a stubbed overlay", () => {
        ...
        cy.get(
          ".file-contents-wrapper > #brace-editor > .ace_scroller > .ace_content > .ace_text-layer > :nth-child(13)"
        ).as("replicaKey");
      ...
      it("allows the stubbed overlay to be edited", () => {
        const backspacesToRemoveToBeModified = buildRepeatKeyString(
          "{backspace}",
          14
        );
        cy.get(".ace_text-input")
          .last()
          .type(`${backspacesToRemoveToBeModified}10`, { force: true });
      });
      context("valid Kustomize overlay written", () => {
        ...
        it("allows navigation to the overlay finalization step", () => {
          cy.get(".secondary").click();
          cy.location("pathname", { timeout: 30000 }).should("eq", "/outro");
        });
      });
    });
  });
